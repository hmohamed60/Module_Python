...
def generate_password(length, lowercase_count, uppercase_count, digit_count, special_count):
    # On vérifions si la somme des compteurs n'excède pas la longueur du mot de passe
    ...
    # Nous générons les caractères de chaque catégorie
    ...
    # Nous complétons la liste pour atteindre la longueur souhaitée
    remaining_chars = length - total_count
    all_chars = lowercase_chars + uppercase_chars + digit_chars + special_chars
    all_chars += ''.join(random.choice(all_chars) for _ in range(remaining_chars))

    # On mélange les caractères pour obtenir un mot de passe aléatoire
